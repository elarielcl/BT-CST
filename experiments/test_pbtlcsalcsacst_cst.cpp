...
int main(int argc, char *argv[]) {
    ...
    PBTLCSALCSACST * pbtlcsalcsacst = new PBTLCSALCSACST(base_name, input_string, version, r, mll, c);
    pbtlcsalcsacst->print_statistics();
    ...
    for (int node : first_sampling) {
        pbtlcsalcsacst->pbtcTree_->first_child(node);
    }
    getrusage(RUSAGE_SELF, &usage);
    ...
    ru_start = usage.ru_utime;
    for (int node : first_sampling) {
        pbtlcsalcsacst->node_depth(node);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    ru_start = usage.ru_utime;
    for (int node : first_sampling) {
        pbtlcsalcsacst->next_sibling(node);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    for (int node: fifth_sampling) {
        int depth = pbtlcsalcsacst->node_depth(node) - 1;
    ...
    for (int i = 0; i < fifth_sampling.size(); ++i) {
        pbtlcsalcsacst->pbtcTree_->level_ancestor(fifth_sampling[i], depths[i]);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    for (std::pair<int,int> nodes : fourth_sampling) {
        pbtlcsalcsacst->lca(nodes.first, nodes.second);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    for (int node : third_sampling) {
        pbtlcsalcsacst->suffix_link(node);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    ru_start = usage.ru_utime;
    for (int node : first_sampling) {
        pbtlcsalcsacst->string_depth(node);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    for (int node: sixth_sampling) {
        int depth = pbtlcsalcsacst->string_depth(node) + 1 ; ///CHECK THIS
    ...
    for (int i = 0; i < sixth_sampling.size(); ++i) {
        pbtlcsalcsacst->string_ancestor(sixth_sampling[i], sdepths[i]);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
...
/*
    std::vector<int> fifth_sampling;
    for (int node : first_sampling) {
        if (!pbtlcsalcsacst->is_leaf(node)) fifth_sampling.push_back(node);
    }
...
*/
    ...
    for (int i = 0; i < second_sampling.size(); ++i) {
        pbtlcsalcsacst->labeled_child(second_sampling[i], letters[i]);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
