...
#include <lcsa/repair_differential_array_with_compressed_lengths.hpp>
...
int main(int argc, char **argv) {
    ...
    repair_differential_array_with_compressed_lengths::repair_differential_array_with_compressed_lengths dsa(csa.sa_sample, sampling_rate);

    sdsl::int_vector<> isa(csa.size(),0);
    for (int i = 0; i < isa.size(); ++i) {
        isa[csa[i]] = i;
    }
    repair_differential_array_with_compressed_lengths::repair_differential_array_with_compressed_lengths disa(isa, sampling_rate);
    repair_differential_array_with_compressed_lengths::repair_differential_array_with_compressed_lengths dlcp(lcp, sampling_rate);
