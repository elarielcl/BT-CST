...
int main(int argc, char *argv[]) {
    ...
    std::cout << "SCT3 SIZE: " << sdsl::size_in_bytes(sct3) << std::endl;
    std::cout << "SCT3 CSA: " << sdsl::size_in_bytes(sct3.csa) << std::endl;
    std::cout << "SCT3 LCP: " << sdsl::size_in_bytes(sct3.lcp) << std::endl;
    ...
    for (auto nodes : fourth_sampling_sct3) {
        sct3.lca(nodes.first, nodes.second);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    for (int i = 0; i < sixth_sampling_sct3.size(); ++i) {
        ...
        while (node != sct3.root()) {
            if (sct3.depth(node) < sdepth) break;
            node = sct3.parent(node);
        }
        // Implementations of sancestor
    }
    ...
    for (int i = 0; i < second_sampling_sct3.size(); ++i) {
        sct3.child(second_sampling_sct3[i], letters[i]);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
