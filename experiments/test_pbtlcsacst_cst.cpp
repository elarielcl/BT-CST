...
int main(int argc, char *argv[]) {
    ...
    PBTLCSACST * pbtlcsacst = new PBTLCSACST(base_name, input_string, version, r, mll, c);
    pbtlcsacst->print_statistics();
    ...
    for (int node : first_sampling) {
        pbtlcsacst->pbtcTree_->first_child(node);
    }
    getrusage(RUSAGE_SELF, &usage);
    ...
    for (int node: fifth_sampling) {
        int depth = pbtlcsacst->node_depth(node) - 1;
    ...
    for (int i = 0; i < fifth_sampling.size(); ++i) {
        pbtlcsacst->pbtcTree_->level_ancestor(fifth_sampling[i], depths[i]);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    for (std::pair<int,int> nodes : fourth_sampling) {
        pbtlcsacst->lca(nodes.first, nodes.second);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
    ...
    for (int node: sixth_sampling) {
        int depth = pbtlcsacst->string_depth(node) + 1 ; ///CHECK THIS
    ...
    for (int i = 0; i < sixth_sampling.size(); ++i) {
        pbtlcsacst->string_ancestor(sixth_sampling[i], sdepths[i]);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
...
/*
    std::vector<int> fifth_sampling;
    for (int node : first_sampling) {
        if (!pbtlcsacst->is_leaf(node)) fifth_sampling.push_back(node);
    }
...
*/
    ...
    for (int i = 0; i < second_sampling.size(); ++i) {
        pbtlcsacst->labeled_child(second_sampling[i], letters[i]);
    }
    getrusage(RUSAGE_SELF, &usage);
    rs_end = usage.ru_stime;
    ru_end = usage.ru_utime;
