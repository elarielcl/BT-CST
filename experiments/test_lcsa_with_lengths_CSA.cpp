...
int main(int argc, char **argv) {
    ...
    repair_differential_array_with_lengths::repair_differential_array_with_lengths dsa(csa.sa_sample, sampling_rate);

    sdsl::int_vector<> isa(csa.size(),0);
    for (int i = 0; i < isa.size(); ++i) {
        isa[csa[i]] = i;
    }
    repair_differential_array_with_lengths::repair_differential_array_with_lengths disa(isa, sampling_rate);
    repair_differential_array_with_lengths::repair_differential_array_with_lengths dlcp(lcp, sampling_rate);
