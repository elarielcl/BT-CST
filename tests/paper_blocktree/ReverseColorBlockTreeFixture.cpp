...
class ReverseColorBlockTreeFixture : public ::testing::TestWithParam<::testing::tuple<int, int, std::string, int>> {
protected:
...
public:
    ...
    ReverseColorBlockTreeFixture() : ::testing::TestWithParam<::testing::tuple<int, int, std::string, int>>() {
    ...
    virtual ~ReverseColorBlockTreeFixture() {
        delete block_tree_;
...
TEST_P(ReverseColorBlockTreeFixture, color_property_check) {
    EXPECT_LE(block_tree_->get_max_hop(), c_);
}


// This test checks that back blocks don't point to themselves
TEST_P(ReverseColorBlockTreeFixture, no_self_references_check) {
...
TEST_P(ReverseColorBlockTreeFixture, text_consecutive_pointed_blocks_check) {
    std::vector<Block*> level = {block_tree_->root_block_};
    for (std::vector<Block*> level : block_tree_->levelwise_iterator()) {
        for (Block* b: level) {
