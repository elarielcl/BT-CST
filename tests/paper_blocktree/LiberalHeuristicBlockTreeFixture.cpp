...
class LiberalHeuristicBlockTreeFixture : public ::testing::TestWithParam<::testing::tuple<int, int, std::string>> {
protected:
...
public:
    ...
    LiberalHeuristicBlockTreeFixture() : ::testing::TestWithParam<::testing::tuple<int, int, std::string>>() {
    ...
    virtual ~LiberalHeuristicBlockTreeFixture() {
        delete block_tree_;
...
TEST_P(LiberalHeuristicBlockTreeFixture, no_self_references_check) {
...
TEST_P(LiberalHeuristicBlockTreeFixture, text_consecutive_pointed_blocks_check) {
    std::vector<Block*> level = {block_tree_->root_block_};
    for (std::vector<Block*> level : block_tree_->levelwise_iterator()) {
        for (Block* b: level) {
