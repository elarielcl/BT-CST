...
class OtherBackFrontBlockTreeFixture : public ::testing::TestWithParam<::testing::tuple<int, int, std::string>> {
protected:
...
public:
    ...
    OtherBackFrontBlockTreeFixture() : ::testing::TestWithParam<::testing::tuple<int, int, std::string>>() {
    ...
    virtual ~OtherBackFrontBlockTreeFixture() {
        delete block_tree_;
...
TEST_P(OtherBackFrontBlockTreeFixture, no_self_references_check) {
...
TEST_P(OtherBackFrontBlockTreeFixture, no_double_pointer_check) {
    std::vector<Block*> level = {block_tree_->root_block_};
    for (std::vector<Block*> level : block_tree_->levelwise_iterator()) {
        for (Block* b: level) {
            if (dynamic_cast<BackBlock*>(b)) {
...
// is correctly set
TEST_P(OtherBackFrontBlockTreeFixture, pointing_to_me_field_check) {
    std::vector<Block*> level = {block_tree_->root_block_};
    for (std::vector<Block*> level : block_tree_->levelwise_iterator()) {
        for (Block* b: level) {
            if (dynamic_cast<BackBlock*>(b)) {
...
TEST_P(OtherBackFrontBlockTreeFixture, text_consecutive_pointed_blocks_check) {
    std::vector<Block*> level = {block_tree_->root_block_};
    for (std::vector<Block*> level : block_tree_->levelwise_iterator()) {
        for (Block* b: level) {
