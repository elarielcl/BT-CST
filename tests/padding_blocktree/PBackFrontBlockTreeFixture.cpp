...
class PBackFrontBlockTreeFixture : public ::testing::TestWithParam<::testing::tuple<int, int, std::string>> {
protected:
...
public:
    ...
    PBackFrontBlockTreeFixture() : ::testing::TestWithParam<::testing::tuple<int, int, std::string>>() {
    ...
    virtual ~PBackFrontBlockTreeFixture() {
        delete block_tree_;
...
TEST_P(PBackFrontBlockTreeFixture, no_self_references_check) {
...
TEST_P(PBackFrontBlockTreeFixture, no_double_pointer_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
            if (dynamic_cast<PBackBlock*>(b)) {
...
// is correctly set
TEST_P(PBackFrontBlockTreeFixture, pointing_to_me_field_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
            if (dynamic_cast<PBackBlock*>(b)) {
...
TEST_P(PBackFrontBlockTreeFixture, text_consecutive_pointed_blocks_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
