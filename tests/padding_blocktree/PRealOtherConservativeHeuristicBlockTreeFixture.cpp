...
class PRealOtherConservativeHeuristicBlockTreeFixture : public ::testing::TestWithParam<::testing::tuple<int, int, std::string>> {
protected:
...
public:
    ...
    PRealOtherConservativeHeuristicBlockTreeFixture() : ::testing::TestWithParam<::testing::tuple<int, int, std::string>>() {
    ...
    virtual ~PRealOtherConservativeHeuristicBlockTreeFixture() {
        delete block_tree_;
...
TEST_P(PRealOtherConservativeHeuristicBlockTreeFixture, no_self_references_check) {
...
TEST_P(PRealOtherConservativeHeuristicBlockTreeFixture, no_double_pointer_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
            if (dynamic_cast<PBackBlock*>(b)) {
...
// is correctly set
TEST_P(PRealOtherConservativeHeuristicBlockTreeFixture, pointing_to_me_field_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
            if (dynamic_cast<PBackBlock*>(b)) {
...
TEST_P(PRealOtherConservativeHeuristicBlockTreeFixture, text_consecutive_pointed_blocks_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
