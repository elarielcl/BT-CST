...
class PReverseColorBlockTreeFixture : public ::testing::TestWithParam<::testing::tuple<int, int, std::string, int>> {
protected:
...
public:
    ...
    PReverseColorBlockTreeFixture() : ::testing::TestWithParam<::testing::tuple<int, int, std::string, int>>() {
        ...
        block_tree_->process_back_pointers_reverse_conservative_heuristic(c_);
    ...
    virtual ~PReverseColorBlockTreeFixture() {
        delete block_tree_;
...
TEST_P(PReverseColorBlockTreeFixture, color_property_check) {
    EXPECT_LE(block_tree_->get_max_hop(), c_);
}


// This test checks that back blocks don't point to themselves
TEST_P(PReverseColorBlockTreeFixture, no_self_references_check) {
...
TEST_P(PReverseColorBlockTreeFixture, text_consecutive_pointed_blocks_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
