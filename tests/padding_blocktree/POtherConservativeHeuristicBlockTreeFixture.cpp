...
class POtherConservativeHeuristicBlockTreeFixture : public ::testing::TestWithParam<::testing::tuple<int, int, std::string>> {
protected:
...
public:
    ...
    POtherConservativeHeuristicBlockTreeFixture() : ::testing::TestWithParam<::testing::tuple<int, int, std::string>>() {
    ...
    virtual ~POtherConservativeHeuristicBlockTreeFixture() {
        delete block_tree_;
...
TEST_P(POtherConservativeHeuristicBlockTreeFixture, no_self_references_check) {
...
TEST_P(POtherConservativeHeuristicBlockTreeFixture, no_double_pointer_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
            if (dynamic_cast<PBackBlock*>(b)) {
...
// is correctly set
TEST_P(POtherConservativeHeuristicBlockTreeFixture, pointing_to_me_field_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
            if (dynamic_cast<PBackBlock*>(b)) {
...
TEST_P(POtherConservativeHeuristicBlockTreeFixture, text_consecutive_pointed_blocks_check) {
    std::vector<PBlock*> level = {block_tree_->root_block_};
    for (std::vector<PBlock*> level : block_tree_->levelwise_iterator()) {
        for (PBlock* b: level) {
