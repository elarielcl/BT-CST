...
// This test checks the rank_10_alternative_noback method for every
// position in the input
TEST_P(BPSPCBlockTreeFixture, rank_10_alternative_noback_check) {
    int r = 0;
    bool one_seen = false;
    for (int i = 0; i < input_.length(); ++i) {
        if (input_[i] == input_[0]) {
        ...
        EXPECT_EQ(bps_->rank_10_alternative_noback(i), r);
    }
}
...
// This test checks the select_10_alternative_noback method for every
// position in the input
TEST_P(BPSPCBlockTreeFixture, select_10_alternative_noback_check) {
    int r = 0;
    bool one_seen = false;
    for (int i = 0; i < input_.length(); ++i) {
        if (input_[i] == input_[0]) {
            ...
            if (one_seen) {
                ++r;
                EXPECT_EQ(bps_->select_10_alternative_noback(r), i-1);
            }
            one_seen = false;
        }
    }
...
// This test checks the my_fwdsearch_cback method for every character
// in the input and d = {-1, -2} works correctly
TEST_P(BPSPCBlockTreeFixture, my_fwdsearch_cback_check) {
    std::unordered_set<int> ds = {-1, -2};
    for (int d : ds) {
        for (int i = 0; i < input_.length(); ++i) {
            int search = bps_->my_fwdsearch_cback(i, d);
            int excess = 0;
            int j = i;
            while (true) {
                ++j;
...
// This test checks the my_fwdsearch_nobackmin method for every character
// in the input and d = {-1, -2} works correctly
TEST_P(BPSPCBlockTreeFixture, my_fwdsearch_nobackmin_check) {
    std::unordered_set<int> ds = {-1, -2};
    for (int d : ds) {
        for (int i = 0; i < input_.length(); ++i) {
            int search = bps_->my_fwdsearch_nobackmin(i, d);
            int excess = 0;
            int j = i;
            while (true) {
                ++j;
...
// This test checks the my_bwdsearch_nobackmin method for every character
// in the input and d = {-1, -2} works correctly
TEST_P(BPSPCBlockTreeFixture, my_bwdsearch_nobackmin_check) {
    std::unordered_set<int> ds = {-1, -2};
    for (int d : ds) {
        for (int i = 0; i < input_.length(); ++i) {
            int search = bps_->my_bwdsearch_nobackmin(i, d);
            int excess = 0;
            int j = i;
            while (true) {
                if (j == 0) break;
...
// This test checks the my_positive_fwdsearch_nobackmax method for every character
// in the input and d = {1, 2} works correctly
TEST_P(BPSPCBlockTreeFixture, my_positive_fwdsearch_nobackmax_check) {
    std::unordered_set<int> ds = {1, 2};
    for (int d : ds) {
        for (int i = 0; i < input_.length(); ++i) {
            int search = bps_->my_positive_fwdsearch_nobackmax(i, d);
            int excess = 0;
            int j = i;
            while (true) {
                ++j;
...
// This test checks the my_positive_bwdsearch_nobackmax method for every character
// in the input and d = {1,2} works correctly
TEST_P(BPSPCBlockTreeFixture, my_positive_bwdsearch_nobackmax_check) {
    std::unordered_set<int> ds = {1, 2};
    for (int d : ds) {
        for (int i = 0; i < input_.length(); ++i) {
            int search = bps_->my_positive_bwdsearch_nobackmax(i, d);
            int excess = 0;
            int j = i;
            while (true) {
                if (j == 0) break;
...
// This test checks the my_min_excess_cback(i,j) method for some sample
TEST_P(BPSPCBlockTreeFixture, my_min_excess_cback_check) {
    for (int j = 0; j < input_.length(); j = 2*j+1) {
        for (int i = 0; i <= j; i = 2*i+1) {
            EXPECT_EQ(block_tree_->root_block_->linear_min_excess(i,j), bps_->my_min_excess_cback(i,j));
        }
    }
}

// This test checks the my_min_excess_nobackmin(i,j) method for some sample
TEST_P(BPSPCBlockTreeFixture, my_min_excess_nobackmin_check) {
    for (int j = 0; j < input_.length(); j = 2*j+1) {
        for (int i = 0; i <= j; i = 2*i+1) {
            EXPECT_EQ(block_tree_->root_block_->linear_min_excess(i,j), bps_->my_min_excess_nobackmin(i,j));
        }
    }
}
...
// This test checks the my_max_excess_nobackmax(i,j) method for some sample
TEST_P(BPSPCBlockTreeFixture, my_max_excess_nobackmax_check) {
    for (int j = 0; j < input_.length(); j = 2*j+1) {
        for (int i = 0; i <= j; i = 2*i+1) {
            EXPECT_EQ(block_tree_->root_block_->linear_max_excess(i,j), bps_->my_max_excess_nobackmax(i,j));
        }
    }
}
...
// This test checks the my_min_count_nobackmin(i,j) method for some sample
TEST_P(BPSPCBlockTreeFixture, my_min_count_nobackmin_check) {
    for (int j = 0; j < input_.length(); j = 2*j+1) {
        for (int i = 0; i <= j; i = 2*i+1) {
            EXPECT_EQ(block_tree_->root_block_->linear_min_count(i,j), bps_->my_min_count_nobackmin(i,j));
        }
    }
}
...
// This test checks the my_min_select_nobackmin(i,j,t) method for some sample
TEST_P(BPSPCBlockTreeFixture, my_min_select_nobackmin_check) {
    for (int j = 0; j < input_.length(); j = 2*j+1) {
        for (int i = 0; i <= j; i = 2*i+1) {
            int count = block_tree_->min_count(i,j);
            for (int t = 1; t <= count ; ++t) {
                EXPECT_EQ(block_tree_->root_block_->linear_min_select(i,j,t), bps_->my_min_select_nobackmin(i,j,t));
            }
        }
    }
}
