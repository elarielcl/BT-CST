...
class PBTISARLCSACSTFixture : public ::testing::TestWithParam<std::string> { // Some tests for this class encoded for an specific input banana. Also, this doesn't check topology specifics methods
protected:
    ...
    virtual void SetUp() {
        ...
        buffer << t.rdbuf();
        input_= buffer.str();
        cst_ = new PBTISARLCSACST(input_);
...
public:
    ...
    PBTISARLCSACSTFixture() : ::testing::TestWithParam<std::string>() {
    }

    virtual ~PBTISARLCSACSTFixture() {
    }
};
...
TEST_P(PBTISARLCSACSTFixture, string_depth_check) {
...
TEST_P(PBTISARLCSACSTFixture, labels_check) {
    ...
    for (int i = 0; i < labels.size(); ++i) {
        ...
        for (int j = 0; j < string_depths[i]-1; ++j) {
            if (labels[i][j] != cst_->string(node, j))
                std::cout << cst_->string(node, j);
...
TEST_P(PBTISARLCSACSTFixture, labeled_child_check) {
...
TEST_P(PBTISARLCSACSTFixture, suffix_link_check) {
...
TEST_P(PBTISARLCSACSTFixture, weiner_link_check) {
...
TEST_P(PBTISARLCSACSTFixture, locus_check) {
...
TEST_P(PBTISARLCSACSTFixture, string_ancestor_check) {
    EXPECT_EQ(cst_->pbtcTree_->node_select(5),
