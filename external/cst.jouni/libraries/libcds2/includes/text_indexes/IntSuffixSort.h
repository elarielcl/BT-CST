...
#define MED3(a, b, c)   (KEY(a)<KEY(b) ?                        \
    ...
    template<typename T> class IntSuffixSort {
    protected:
        static T *I;                 /* group array, ultimately suffix array.*/
        static T *V;                          /* inverse array, ultimately inverse of I.*/
        static T r;                           /* number of symbols aggregated by transform.*/
        static T h;                           /* length of already-sorted prefixes.*/
...
//        static T   h = -1;
        ...
        static void update_group(T *pl, T *pm);
        ...
        static void select_sort_split(T *p, ulong n);

        /* Subroutine for sort_split, algorithm by Bentley & McIlroy.*/
        static T choose_pivot(T *p, ulong n);
        ...
        static void sort_split(T *p, ulong n);
        ...
        static void bucketsort(T *x, T *p, ulong n, T k);
        ...
        static T transform(T *x, T *p, ulong n, T k, T l, int q);
    ...
    public:
        ...
        static void suffixsort(T *x, T *p, ulong n, T k, T l);

    };



    template <typename T> T* IntSuffixSort<T>::I = nullptr;
    template <typename T> T* IntSuffixSort<T>::V = nullptr;
    template <typename T> T IntSuffixSort<T>::r = -1;
    template <typename T> T IntSuffixSort<T>::h = -1;

    template <typename T> void IntSuffixSort<T>::update_group(T *pl, T *pm){
        size_t g;
        g = pm - I;                      /* group number.*/
    ...
    /* Quadratic sorting method to use for small subarrays. To be able to update
       group numbers consistently, a variant of selection sorting is used.*/
    template <typename T> void IntSuffixSort<T>::select_sort_split(T *p, ulong n){
        T *pa, *pb, *pi, *pn, tmp;
        T f, v;
    ...
    /* Subroutine for sort_split, algorithm by Bentley & McIlroy.*/
    template <typename T> T IntSuffixSort<T>::choose_pivot(T *p, ulong n){
        T *pl, *pm, *pn;
        size_t s;
    ...
    template <typename T> void IntSuffixSort<T>::sort_split(T *p, ulong n){
        T *pa, *pb, *pc, *pd, *pl, *pm, *pn,tmp,f,v;
        size_t s, t;
    ...
    template <typename T> void IntSuffixSort<T>::bucketsort(T *x, T *p, ulong n, T k){
        T *pi, d, g, i;
        size_t c;
    ...
    template <typename T> T IntSuffixSort<T>::transform(T *x, T *p, ulong n, T k, T l, int q){
        int b, c, d, e, i, j, m, s;
        T *pi, *pj;
    ...
    template <typename T> void IntSuffixSort<T>::reset(){
        I=nullptr;
        V=nullptr;
        r = -1;
        h = -1;
    ...
    //TODO: replace the ints by its corresponding type. Note there are accesses to INT_MAX and stuff
    //like that, which implies take them into account when I change the type of that variables.
    template <typename T> void IntSuffixSort<T>::suffixsort(T *x, T *p, ulong n, T k, T l){
        T *pi, *pk;
        int i, j, s, sl;
        ...
        if (n >=(int) k - l) {                /* if bucketing possible,*/
