...
void deleteHash (Thash_repair *H, int id) // deletes H->Rec[id].pair from hash
...
Thash_repair createHash (int maxpos, Trarray_repair *Rec)
...
static int finsertHash (Thash_repair H, Tpair_repair p) 
...
void insertHash (Thash_repair *H, int id) // inserts H->Rec[id].pair in hash 
  ...
  { int k;
	...
	{ Thash_repair newH = createHash((H->maxpos<<1)|1,H->Rec);
	  int i;
	  int *tab = H->table;
...
void destroyHash (Thash_repair *H)
...
void hashRepos (Thash_repair *H, int id)
