...
Theap_repair createHeap (int u, Trarray_repair *Rec, float factor, int minsize) 
...
void destroyHeap (Theap_repair *H) // destroys H
...
static void move (Tarray A, int i, int j, Trecord_repair *rec)
...
void prnH (Theap_repair *H)

  { Thfreq_repair *f;
    static int X = 0;
    long prevf = (((long)1)<<(8*sizeof(long)-2))-1;
    int fp = H->largest;
    if (fp == -1) return;
    X++;
    printf ("Heap %i = \n",X);
...
void incFreq (Theap_repair *H, int id) // inc freq of pair Rec[id]

  { Trecord_repair *rec = H->Rec->records;
    long freq = rec[id].freq++;
...
void decFreq (Theap_repair *H, int id) // dec freq of pair Rec[id]
...
void insertHeap (Theap_repair *H, int id)  // with freq 1
...
void heapRepos (Theap_repair *H, int id) // repositions pair
