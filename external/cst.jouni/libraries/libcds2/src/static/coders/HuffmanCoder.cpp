...
namespace cds_static
{
	...
	HuffmanCoder *HuffmanCoder::getFromFreqs(uint *freqs, uint nfreqs){
		HuffmanCoder *ret = new HuffmanCoder();
		ret->huff_table = createHuff(freqs, nfreqs-1);
		return ret;
	}
	...
	uint HuffmanCoder::getMaxv() const{
		return getMax_v(huff_table);
	}
