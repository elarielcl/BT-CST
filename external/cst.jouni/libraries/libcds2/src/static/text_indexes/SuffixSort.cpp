...
#if 0
#include <text_indexes/SuffixSort.h>
...
/* Subroutine for select_sort_split and sort_split. Sets group numbers for a
   group whose lowest position in I is pl and highest position is pm.*/
        ...
        void SuffixSort::update_group(int *pl, int *pm) {
            int g;

            g = pm - SuffixSort::I;                      /* group number.*/
...
/* Quadratic sorting method to use for small subarrays. To be able to update
   group numbers consistently, a variant of selection sorting is used.*/
        ...
        void SuffixSort::select_sort_split(int *p, int n) {
            int *pa, *pb, *pi, *pn;
            int f, v, tmp;
...
/* Subroutine for sort_split, algorithm by Bentley & McIlroy.*/
        ...
        int SuffixSort::choose_pivot(int *p, int n) {
            int *pl, *pm, *pn;
            int s;
...
/* Sorting routine called for each unsorted group. Sorts the array of integers
   ...
   function is based on Program 7.*/
        ...
        void SuffixSort::sort_split(int *p, int n) {
...
/* Bucketsort for first iteration.
   ...
   suffix sorting algorithm.*/
        ...
        void SuffixSort::bucketsort(int *x, int *p, int n, int k) {
            int *pi, i, c, d, g;
...
/* Transforms the alphabet of x by attempting to aggregate several symbols into
   ...
   set to the number of old symbols grouped into one. Only x[n] is 0.*/
        ...
        int SuffixSort::transform(int *x, int *p, int n, int k, int l, int q) {
            int b, c, d, e, i, j, m, s;
            int *pi, *pj;
...
/* Makes suffix array p of x. x becomes inverse of p. p and x are both of size
   ...
   contents of x[n] is disregarded, the n-th symbol being regarded as
        ...
        void SuffixSort::suffixsort(int *x, int *p, int n, int k, int l) {
            int *pi, *pk;
            int i, j, s, sl;
            ...
            if (n >= k - l) {                /* if bucketing possible,*/
                j = transform(V, SuffixSort::I, n, k, l, n);
                bucketsort(V, SuffixSort::I, n, j);   /* bucketsort on first r positions.*/
            } else {
                transform(V, SuffixSort::I, n, k, l, INT_MAX);
                for (i = 0; i <= n; ++i)
                    SuffixSort::I[i] = i;                /* initialize SuffixSort::I with suffix numbers.*/
                h = 0;
                sort_split(SuffixSort::I, n + 1);       /* quicksort on first r positions.*/
            }
            h = r;                         /* number of symbols aggregated by transform.*/

            while (*SuffixSort::I >= -n) {
                pi = SuffixSort::I;                     /* pi is first position of group.*/
                ...
                do {
                    if ((s = *pi) < 0) {
                        ...
                        pk = SuffixSort::I + V[s] + 1;        /* pk-1 is last position of unsorted group.*/
                        sort_split(pi, pk - pi);
                        pi = pk;              /* next group.*/
                    }
                } while (pi <= SuffixSort::I + n);
                if (sl)                   /* if the array ends with a sorted group.*/
                    *(pi + sl) = sl;           /* combine sorted groups at end of SuffixSort::I.*/
