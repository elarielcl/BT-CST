...
namespace cds_static
{
	...
	WaveletTreeNoptrs::WaveletTreeNoptrs(uint * symbols, size_t n, BitSequenceBuilder ** bmb, uint nbmb, Mapper * am, bool deleteSymbols) : Sequence(n) {
		...
		for (uint i = 0; i < height; i++) {
			...
			for (uint j=1;j<nbmb;j++){
				BitSequence *b = bmb[j]->build(_bm[i], new_n);
				if (b->getSize()<bitstring[i]->getSize()){
