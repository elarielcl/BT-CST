...
PBTRLCSACST::PBTRLCSACST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top,
...
PBTRLCSACST::PBTRLCSACST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top, int block_size, int sa_sampling_rate) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
int PBTRLCSACST::root() {
    return 0;
}

int PBTRLCSACST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTRLCSACST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTRLCSACST::string_depth(int node) {
    if (node == 0) return 0;
    /*
...
int PBTRLCSACST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTRLCSACST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTRLCSACST::leftmost_leaf(int node) {
...
int PBTRLCSACST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;
...
int PBTRLCSACST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTRLCSACST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTRLCSACST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTRLCSACST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTRLCSACST::string(int node, int i) {
...
std::string PBTRLCSACST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
...
int PBTRLCSACST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTRLCSACST::bin_search_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
    int child = node+1;
    int j = -1;
...
int PBTRLCSACST::bin_search_count_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
...
int PBTRLCSACST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTRLCSACST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTRLCSACST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTRLCSACST::suffix_link(int node) {
...
int PBTRLCSACST::suffix_link(int node, int i) {
...
/*
int PBTRLCSACST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTRLCSACST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTRLCSACST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return rlcsa_->locate(lr);
}

int PBTRLCSACST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTRLCSACST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTRLCSACST::nodes() {
    return n_/2;
}

int PBTRLCSACST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTRLCSACST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTRLCSACST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTRLCSACST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTRLCSACST::locate(int node) {
...
int PBTRLCSACST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTRLCSACST::print_statistics() {
