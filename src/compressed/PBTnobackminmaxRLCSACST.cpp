...
PBTnobackminmaxRLCSACST::PBTnobackminmaxRLCSACST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top) {
    sdsl::cst_sada<> cst;
    ...
    pbtcTree_ = new PBTCTreenobackminmax(topology, block_tree_version_top, r_top, mll_top, c_top);
...
PBTnobackminmaxRLCSACST::PBTnobackminmaxRLCSACST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
PBTnobackminmaxRLCSACST::~PBTnobackminmaxRLCSACST() {
...
int PBTnobackminmaxRLCSACST::access(int i) {
...
int PBTnobackminmaxRLCSACST::root() {
    return 0;
}

int PBTnobackminmaxRLCSACST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTnobackminmaxRLCSACST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTnobackminmaxRLCSACST::string_depth(int node) {
    if (node == 0) return 0;
    /*
...
int PBTnobackminmaxRLCSACST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTnobackminmaxRLCSACST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTnobackminmaxRLCSACST::leftmost_leaf(int node) {
...
int PBTnobackminmaxRLCSACST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;
...
int PBTnobackminmaxRLCSACST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTnobackminmaxRLCSACST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch_nobackmin(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTnobackminmaxRLCSACST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTnobackminmaxRLCSACST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTnobackminmaxRLCSACST::string(int node, int i) {
    int lr = pbtcTree_->leaf_rank(node);
...
std::string PBTnobackminmaxRLCSACST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
    int sa = rlcsa_->locate(lr-1);
    std::string output = "";
    while (i <= j) {
...
int PBTnobackminmaxRLCSACST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTnobackminmaxRLCSACST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTnobackminmaxRLCSACST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTnobackminmaxRLCSACST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTnobackminmaxRLCSACST::suffix_link(int node) {
...
int PBTnobackminmaxRLCSACST::suffix_link(int node, int i) {
...
/*
int PBTnobackminmaxRLCSACST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTnobackminmaxRLCSACST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTnobackminmaxRLCSACST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return rlcsa_->locate(lr);
}

int PBTnobackminmaxRLCSACST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTnobackminmaxRLCSACST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTnobackminmaxRLCSACST::nodes() {
    return n_/2;
}

int PBTnobackminmaxRLCSACST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTnobackminmaxRLCSACST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTnobackminmaxRLCSACST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTnobackminmaxRLCSACST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTnobackminmaxRLCSACST::locate(int node) {
...
int PBTnobackminmaxRLCSACST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTnobackminmaxRLCSACST::print_statistics() {
