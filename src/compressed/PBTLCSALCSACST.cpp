...
PBTLCSALCSACST::PBTLCSALCSACST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top,
...
PBTLCSALCSACST::PBTLCSALCSACST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top, int block_size, int sa_sampling_rate) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
int PBTLCSALCSACST::access(int i) {
...
int PBTLCSALCSACST::root() {
    return 0;
}

int PBTLCSALCSACST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTLCSALCSACST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTLCSALCSACST::string_depth(int node) {
    if (node == 0) return 0;
    /*
...
int PBTLCSALCSACST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTLCSALCSACST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTLCSALCSACST::leftmost_leaf(int node) {
...
int PBTLCSALCSACST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;
...
int PBTLCSALCSACST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTLCSALCSACST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTLCSALCSACST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTLCSALCSACST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTLCSALCSACST::string(int node, int i) {
...
std::string PBTLCSALCSACST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
    int sa = (*dsa_)[lr-1];
    std::string output = "";
    while (i <= j) {
...
int PBTLCSALCSACST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTLCSALCSACST::bin_search_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
    int child = node+1;
    int j = -1;
...
int PBTLCSALCSACST::bin_search_count_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
...
int PBTLCSALCSACST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTLCSALCSACST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTLCSALCSACST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTLCSALCSACST::suffix_link(int node) {
...
int PBTLCSALCSACST::suffix_link(int node, int i) {
...
/*
int PBTLCSALCSACST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTLCSALCSACST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTLCSALCSACST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return (*dsa_)[lr];
}

int PBTLCSALCSACST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTLCSALCSACST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTLCSALCSACST::nodes() {
    return n_/2;
}

int PBTLCSALCSACST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTLCSALCSACST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTLCSALCSACST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTLCSALCSACST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTLCSALCSACST::locate(int node) {
...
int PBTLCSALCSACST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTLCSALCSACST::print_statistics() {
