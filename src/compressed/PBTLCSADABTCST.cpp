...
#include <compressed/PBTLCSADABTCST.h>
#include <sdsl/suffix_trees.hpp>
#include <compressed/PBTCLCPA.h>
#include <compressed/PBTCISA.h>

PBTLCSADABTCST::PBTLCSADABTCST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top,
    ...
    dsa_ = new repair_differential_array_with_lengths::repair_differential_array_with_lengths(csa.sa_sample, 128); // Fix Sweet point
    pbtcISA_ = new PBTCISA(input_string, 3, 2, 4, 2); // FIX low space point
...
PBTLCSADABTCST::PBTLCSADABTCST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top, int block_size, int sa_sampling_rate) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
int PBTLCSADABTCST::access(int i) {
    //return input_block_tree_->access(i);
    int r = rlcsa_->getT(pbtcISA_->mapped_saccess(i));
    if (r == 0) return '$';
    return r;
}

int PBTLCSADABTCST::root() {
    return 0;
}

int PBTLCSADABTCST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTLCSADABTCST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTLCSADABTCST::string_depth(int node) {
    if (node == 0) return 0;
    /*
...
int PBTLCSADABTCST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTLCSADABTCST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTLCSADABTCST::leftmost_leaf(int node) {
...
int PBTLCSADABTCST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;
...
int PBTLCSADABTCST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTLCSADABTCST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTLCSADABTCST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTLCSADABTCST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTLCSADABTCST::string(int node, int i) {
    if (node == node_ && i == i_+1) {
        ...
        if (i > disa_constant) psi_ = pbtcISA_->mapped_saccess((*dsa_)[lr]+i);
...
std::string PBTLCSADABTCST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
    int sa = (*dsa_)[lr-1];
    std::string output = "";
    while (i <= j) {
...
int PBTLCSADABTCST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTLCSADABTCST::bin_search_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
    int child = node+1;
    int j = -1;
...
int PBTLCSADABTCST::bin_search_count_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
...
int PBTLCSADABTCST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTLCSADABTCST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTLCSADABTCST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTLCSADABTCST::suffix_link(int node) {
...
int PBTLCSADABTCST::suffix_link(int node, int i) {
    if (node == root()) return root();

    int lml = lb(node);
    int next_suf_lml = (*dsa_)[lml] + i;
    if (next_suf_lml >= n_) return root();
    int psi_lml = pbtcISA_->mapped_saccess(next_suf_lml);

    int rml = rb(node);
    int next_suf_rml = (*dsa_)[rml-1] + i;
    if (next_suf_rml >= n_) return root();
    int psi_rml = pbtcISA_->mapped_saccess(next_suf_rml);
...
/*
int PBTLCSADABTCST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTLCSADABTCST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTLCSADABTCST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return (*dsa_)[lr];
}

int PBTLCSADABTCST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTLCSADABTCST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTLCSADABTCST::nodes() {
    return n_/2;
}

int PBTLCSADABTCST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTLCSADABTCST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTLCSADABTCST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTLCSADABTCST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTLCSADABTCST::locate(int node) {
    std::vector<int> positions;
    int init = lb(node);
    int end = rb(node);
    for (int i = init; i< end; ++i) {
        positions.push_back((*dsa_)[i]);
    }
    return positions;
}

int PBTLCSADABTCST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTLCSADABTCST::print_statistics() {
    ...
    std::cout << "Bytes LCSA(SA) " << dsa_->size_in_bytes() << std::endl;
