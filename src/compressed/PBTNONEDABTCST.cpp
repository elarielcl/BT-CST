...
PBTNONEDABTCST::PBTNONEDABTCST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top,
...
PBTNONEDABTCST::PBTNONEDABTCST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top, int block_size, int sa_sampling_rate) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
int PBTNONEDABTCST::access(int i) {
...
int PBTNONEDABTCST::root() {
    return 0;
}

int PBTNONEDABTCST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTNONEDABTCST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTNONEDABTCST::string_depth(int node) {
    if (node == 0) return 0;
    /*
    ...
    if (pbtcTree_->is_leaf_rank(node, lr)) {
        return n_ - rlcsa_->getSA(lr-1);
    }

    int sc = pbtcTree_->bps_->my_fwdsearch(node+1, -1) + 1;
    int i = pbtcTree_->leaf_rank(sc)-1;
    return lcp_rlcsa_->get_LCP(i, rlcsa_->getSA(i));
}

int PBTNONEDABTCST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTNONEDABTCST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTNONEDABTCST::leftmost_leaf(int node) {
...
int PBTNONEDABTCST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;
...
int PBTNONEDABTCST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTNONEDABTCST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTNONEDABTCST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTNONEDABTCST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTNONEDABTCST::string(int node, int i) {
    if (node == node_ && i == i_+1) {
        ...
        if (i > disa_constant) psi_ = pbtcISA_->mapped_saccess(rlcsa_->getSA(lr)+i);
...
std::string PBTNONEDABTCST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
    int sa = rlcsa_->getSA(lr-1);
    std::string output = "";
    while (i <= j) {
        output += access(sa+i);
        i++;
...
int PBTNONEDABTCST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTNONEDABTCST::bin_search_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
    int child = node+1;
    int j = -1;
...
int PBTNONEDABTCST::bin_search_count_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
...
int PBTNONEDABTCST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTNONEDABTCST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTNONEDABTCST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTNONEDABTCST::suffix_link(int node) {
...
int PBTNONEDABTCST::suffix_link(int node, int i) {
    if (node == root()) return root();

    int lml = lb(node);
    int next_suf_lml = rlcsa_->getSA(lml) + i;
    if (next_suf_lml >= n_) return root();
    int psi_lml = pbtcISA_->mapped_saccess(next_suf_lml);

    int rml = rb(node);
    int next_suf_rml = rlcsa_->getSA(rml-1) + i;
...
/*
int PBTNONEDABTCST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTNONEDABTCST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTNONEDABTCST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return rlcsa_->getSA(lr);
}

int PBTNONEDABTCST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTNONEDABTCST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTNONEDABTCST::nodes() {
    return n_/2;
}

int PBTNONEDABTCST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTNONEDABTCST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTNONEDABTCST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTNONEDABTCST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTNONEDABTCST::locate(int node) {
    std::vector<int> positions;
    int init = lb(node);
    int end = rb(node);
    for (int i = init; i< end; ++i) {
        positions.push_back(rlcsa_->getSA(i));
    }
    return positions;
}

int PBTNONEDABTCST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTNONEDABTCST::print_statistics() {
