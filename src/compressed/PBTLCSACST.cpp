...
#include <compressed/PBTLCSACST.h>
#include <sdsl/suffix_trees.hpp>
#include <compressed/PBTCLCPA.h>
#include <compressed/PBTCISA.h>

PBTLCSACST::PBTLCSACST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top,
...
PBTLCSACST::PBTLCSACST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top, int block_size, int sa_sampling_rate) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
int PBTLCSACST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTLCSACST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTLCSACST::string_depth(int node) {
    if (node == 0) return 0;
    /*
...
int PBTLCSACST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTLCSACST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTLCSACST::leftmost_leaf(int node) {
...
int PBTLCSACST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;
...
int PBTLCSACST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTLCSACST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTLCSACST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTLCSACST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTLCSACST::string(int node, int i) {
...
std::string PBTLCSACST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
    int sa = (*dsa_)[lr-1];
    std::string output = "";
    while (i <= j) {
...
int PBTLCSACST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTLCSACST::bin_search_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
    int child = node+1;
    int j = -1;
...
int PBTLCSACST::bin_search_count_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
...
int PBTLCSACST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTLCSACST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTLCSACST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTLCSACST::suffix_link(int node) {
...
int PBTLCSACST::suffix_link(int node, int i) {
...
/*
int PBTLCSACST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTLCSACST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTLCSACST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return (*dsa_)[lr];
}

int PBTLCSACST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTLCSACST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTLCSACST::nodes() {
    return n_/2;
}

int PBTLCSACST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTLCSACST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTLCSACST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTLCSACST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTLCSACST::locate(int node) {
...
int PBTLCSACST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTLCSACST::print_statistics() {
