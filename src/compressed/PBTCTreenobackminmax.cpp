...
PBTCTreenobackminmax::PBTCTreenobackminmax(std::string& input_string, int block_tree_version, int r, int mll, int c) {
...
PBTCTreenobackminmax::~PBTCTreenobackminmax() {
    delete bps_;
}

int PBTCTreenobackminmax::root() {
    return root_;
}

int PBTCTreenobackminmax::first_child(int node) {
    // check whether node+1 is (
    int a = bps_->access(node+1);
    return (a == '(') ? node+1 : -1;
}

int PBTCTreenobackminmax::last_child(int node) {
    // check whether node+1 is (
    int c = bps_->my_fwdsearch_nobackmin(node, -1);
    return bps_->my_bwdsearch_nobackmin(c-1 ,0) + 1;
}

int PBTCTreenobackminmax::next_sibling(int node) {
...
int PBTCTreenobackminmax::previous_sibling(int node) {
    // check whether node != root and node-1 = )
    return bps_->my_bwdsearch_nobackmin(node-1, 0) + 1;
}

int PBTCTreenobackminmax::parent(int node) {
...
bool PBTCTreenobackminmax::is_leaf(int node) {
    return bps_->open_ != bps_->access(node+1);
}


bool PBTCTreenobackminmax::is_leaf_rank(int node, int& leaf_rank) {
    return bps_->is_leaf_rank_alternative(node+1, leaf_rank);
}

int PBTCTreenobackminmax::node_map(int node) {
    return bps_->rank_1_alternative(node);
}


int PBTCTreenobackminmax::node_select(int i) {
    return bps_->select_1_alternative(i);
}

int PBTCTreenobackminmax::postorder(int node) {
    int c = bps_->my_fwdsearch_nobackmin(node, -1);
    return bps_->rank_0_alternative(c);
}

int PBTCTreenobackminmax::postorder_select(int i) {
    int s = bps_->select_0_alternative(i);
    return bps_->my_bwdsearch_nobackmin(s, 0) + 1;
}

int PBTCTreenobackminmax::depth(int node) {
    return bps_->excess_alternative(node);
}

int PBTCTreenobackminmax::subtree_size(int node) {
    int c = bps_->my_fwdsearch_nobackmin(node ,-1);
    return (c-node+1)/2;
}

bool PBTCTreenobackminmax::is_ancestor(int node_a, int node_d) {
    if (node_a > node_d) return false;
    int c = bps_->my_fwdsearch_nobackmin(node_a, -1);
    return node_d <= c;
}

int PBTCTreenobackminmax::level_ancestor(int node, int d) {
    return bps_->my_bwdsearch_nobackmin(node, -d-1) + 1;
}


int PBTCTreenobackminmax::leaf_rank(int node) {
    return bps_->rank_10_alternative(node) + 1;
}

int PBTCTreenobackminmax::number_of_leaves(int node) {
    int c = bps_->my_fwdsearch_nobackmin(node, -1);
    return leaf_rank(c) - leaf_rank(node);
}

int PBTCTreenobackminmax::leaf_select(int node) {
    return bps_->select_10_alternative(node);
}

int PBTCTreenobackminmax::lca(int node1, int node2) {
