...
#include <compressed/PBTcbackRLCSACST.h>
#include <sdsl/suffix_trees.hpp>
#include <compressed/PBTCLCPA.h>
#include <compressed/PBTCISA.h>

PBTcbackRLCSACST::PBTcbackRLCSACST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top) {
    sdsl::cst_sada<> cst;
    ...
    pbtcTree_ = new PBTCTreecback(topology, block_tree_version_top, r_top, mll_top, c_top);
...
PBTcbackRLCSACST::PBTcbackRLCSACST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
PBTcbackRLCSACST::~PBTcbackRLCSACST() {
...
int PBTcbackRLCSACST::access(int i) {
...
int PBTcbackRLCSACST::root() {
    return 0;
}

int PBTcbackRLCSACST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTcbackRLCSACST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTcbackRLCSACST::string_depth(int node) {
    if (node == 0) return 0;
    /*
    ...
    int sc = pbtcTree_->bps_->my_fwdsearch_cback(node+1, -1) + 1;
    return lcp_rlcsa_->get_LCP(pbtcTree_->leaf_rank(sc)-1, index_);
}

int PBTcbackRLCSACST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTcbackRLCSACST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTcbackRLCSACST::leftmost_leaf(int node) {
...
int PBTcbackRLCSACST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;

    int c = pbtcTree_->bps_->my_fwdsearch_cback(node, -1);
    int lr = pbtcTree_->leaf_rank(c);
    return pbtcTree_->leaf_select(lr-1);
}

int PBTcbackRLCSACST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTcbackRLCSACST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch_cback(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTcbackRLCSACST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTcbackRLCSACST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTcbackRLCSACST::string(int node, int i) {
    int lr = pbtcTree_->leaf_rank(node);
...
std::string PBTcbackRLCSACST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
    int sa = rlcsa_->locate(lr-1);
    std::string output = "";
    while (i <= j) {
...
int PBTcbackRLCSACST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTcbackRLCSACST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTcbackRLCSACST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTcbackRLCSACST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTcbackRLCSACST::suffix_link(int node) {
...
int PBTcbackRLCSACST::suffix_link(int node, int i) {
...
/*
int PBTcbackRLCSACST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTcbackRLCSACST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTcbackRLCSACST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return rlcsa_->locate(lr);
}

int PBTcbackRLCSACST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTcbackRLCSACST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTcbackRLCSACST::nodes() {
    return n_/2;
}

int PBTcbackRLCSACST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTcbackRLCSACST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTcbackRLCSACST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTcbackRLCSACST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTcbackRLCSACST::locate(int node) {
...
int PBTcbackRLCSACST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTcbackRLCSACST::print_statistics() {
