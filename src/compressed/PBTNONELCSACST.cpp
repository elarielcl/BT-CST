...
#include <compressed/PBTNONELCSACST.h>
#include <sdsl/suffix_trees.hpp>
#include <compressed/PBTCLCPA.h>
#include <compressed/PBTCISA.h>

PBTNONELCSACST::PBTNONELCSACST(std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top,
...
PBTNONELCSACST::PBTNONELCSACST(std::string& base_name, std::string& input_string, int block_tree_version_top, int r_top, int mll_top, int c_top, int block_size, int sa_sampling_rate) {
    sdsl::cst_sada<> cst;
    construct_im(cst, input_string, 1);
    std::string topology;
    for (int i = 0; i < cst.bp.size(); ++i) {
...
int PBTNONELCSACST::access(int i) {
...
int PBTNONELCSACST::root() {
    return 0;
}

int PBTNONELCSACST::is_leaf(int node) {
    return pbtcTree_->is_leaf(node);
}

int PBTNONELCSACST::select_leaf(int i) {
    return pbtcTree_->leaf_select(i);
}

int PBTNONELCSACST::string_depth(int node) {
    if (node == 0) return 0;
    /*
...
int PBTNONELCSACST::node_depth(int node) {
    return pbtcTree_->depth(node);
}

int PBTNONELCSACST::size(int node) {
    return pbtcTree_->number_of_leaves(node);
}

int PBTNONELCSACST::leftmost_leaf(int node) {
...
int PBTNONELCSACST::rightmost_leaf(int node) {
    if (pbtcTree_->is_leaf(node)) return node;
...
int PBTNONELCSACST::lb(int node) {
    return pbtcTree_->leaf_rank(node)-1;
}

int PBTNONELCSACST::rb(int node) {
    int c = pbtcTree_->bps_->my_fwdsearch(node, -1);
    return pbtcTree_->leaf_rank(c) - 1;
}

int PBTNONELCSACST::parent(int node) {
    return pbtcTree_->parent(node);
}

int PBTNONELCSACST::next_sibling(int node) {
    return pbtcTree_->next_sibling(node);
}

int PBTNONELCSACST::string(int node, int i) {
    if (node == node_ && i == i_+1) {
        ...
        if (i > disa_constant) psi_ = (*disa_)[rlcsa_->getSA(lr)+i];
...
std::string PBTNONELCSACST::string(int node, int i, int j) { //Maybe path extraction would be util here
    int lr = pbtcTree_->leaf_rank(node);
...
int PBTNONELCSACST::labeled_child(int node, int c) {
    if (c == 0) c = '$';
...
int PBTNONELCSACST::bin_search_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
    int child = node+1;
    int j = -1;
...
int PBTNONELCSACST::bin_search_count_labeled_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
...
int PBTNONELCSACST::select_child(int node, int t) {
    int child = node + 1;
    while (--t) {
        child = pbtcTree_->next_sibling(child);
    }
...
int PBTNONELCSACST::edge(int node, int i) {
    int parent_depth = string_depth(parent(node));
    return string(node, parent_depth+i);
}

int PBTNONELCSACST::lca(int node1, int node2) {
    return pbtcTree_->lca(node1, node2);
}

int PBTNONELCSACST::suffix_link(int node) {
...
int PBTNONELCSACST::suffix_link(int node, int i) {
...
/*
int PBTNONELCSACST::psi(int i) {
    int r = rlcsa_->psi(i) -1;
    return r == -1 ? pa_ : r;
}
 */
/*
int PBTNONELCSACST::weiner_link(int node, int c) { //REIMPLEMENT WITH rlcsa
...
int PBTNONELCSACST::sn(int node) {
    int lr = pbtcTree_->leaf_rank(node);
    return rlcsa_->getSA(lr);
}

int PBTNONELCSACST::id(int node) {
    return pbtcTree_->node_map(node)-1;
}

int PBTNONELCSACST::inv_id(int id) {
    return pbtcTree_->node_select(id+1);
}

int PBTNONELCSACST::nodes() {
    return n_/2;
}

int PBTNONELCSACST::node(int lb, int rb) {
    return lca(select_leaf(lb+1), select_leaf(rb+1));
}

int PBTNONELCSACST::degree(int node) {
    return pbtcTree_->number_of_children(node);
}

int PBTNONELCSACST::count(int node) {
    pbtcTree_->number_of_leaves(node);
}


int PBTNONELCSACST::locus(std::string pattern) {
    int init = 0;
    int end = n_-1;
    bool flag = false;
...
std::vector<int> PBTNONELCSACST::locate(int node) {
...
int PBTNONELCSACST::string_ancestor(int node, int d) {
    /*
    if (d==0) return root();
    if (v==root()) return 0;
...
void PBTNONELCSACST::print_statistics() {
